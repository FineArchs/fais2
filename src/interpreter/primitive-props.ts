...
const PRIMITIVE_PROPS: PrimitiveProps = {
	...
	str: {
		to_num: (target: VStr): VFn => FN_NATIVE((_, _opts) => {
			const parsed = parseInt(target.value, 10);
			if (isNaN(parsed)) return NULL;
			return NUM(parsed);
		}),

		to_arr: (target: VStr): VFn => FN_NATIVE((_, _opts) => {
			return ARR(toArray(target.value).map(s => STR(s)));
		}),

		to_unicode_arr: (target: VStr): VFn => FN_NATIVE((_, _opts) => {
			return ARR([...target.value].map((s) => STR(s)));
		}),

		to_unicode_codepoint_arr: (target: VStr): VFn => FN_NATIVE((_, _opts) => {
		...
		to_char_arr: (target: VStr): VFn => FN_NATIVE((_, _opts) => {
			return ARR(target.value.split('').map((s) => STR(s)));
		}),

		to_charcode_arr: (target: VStr): VFn => FN_NATIVE((_, _opts) => {
			return ARR(target.value.split('').map((s) => NUM(s.charCodeAt(0))));
		}),

		to_utf8_byte_arr: (target: VStr): VFn => FN_NATIVE((_, _opts) => {
		...
		trim: (target: VStr): VFn => FN_NATIVE((_, _opts) => {
			return STR(target.value.trim());
		}),

		upper: (target: VStr): VFn => FN_NATIVE((_, _opts) => {
			return STR(target.value.toUpperCase());
		}),

		lower: (target: VStr): VFn => FN_NATIVE((_, _opts) => {
	...
	arr: {
		...
		push: (target: VArr): VFn => FN_NATIVE(([val], _opts) => {
			expectAny(val);
			target.value.push(val);
			return target;
		}),

		unshift: (target: VArr): VFn => FN_NATIVE(([val], _opts) => {
			expectAny(val);
			target.value.unshift(val);
			return target;
		}),

		pop: (target: VArr): VFn => FN_NATIVE((_, _opts) => {
			return target.value.pop() ?? NULL;
		}),

		shift: (target: VArr): VFn => FN_NATIVE((_, _opts) => {
		...
		sort: (target: VArr): VFn => FN_NATIVE(async ([comp], opts) => {
			...
			const merge = (left: Value[], right: Value[], comp: VFn): Value[] => {
				const result: Value[] = [];
				let leftIndex = 0;
				let rightIndex = 0;
				while (leftIndex < left.length && rightIndex < right.length) {
